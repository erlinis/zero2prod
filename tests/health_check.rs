...
use zero2prod::{
    ...
    telemetry::{get_tracing_subscriber, init_tracing_susbcriber},
};
...
static TRACING: Lazy<()> = Lazy::new(|| {
    ...
    let subscriber_name = "test-log".to_string();
...
async fn spawn_app() -> TestApp {
    ...
    let db_pool = configurate_database(&configuration.database).await;

    let server = zero2prod::startup::run(listener, db_pool.clone()).expect("Failed to bind adress");
